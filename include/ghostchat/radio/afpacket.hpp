...
class AFPacketRadio : public Radio {
  ...
  private:
    bool raw_send(const std::vector<std::uint8_t> &frame, const std::uint8_t dst_mac[ETH_ALEN]);
    ...
    std::unordered_map<std::uint64_t, std::vector<std::vector<std::uint8_t>>> pending_unknown_;
